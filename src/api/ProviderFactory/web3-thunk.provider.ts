  ...
  return <Returned, ThunkArg = any>(
    ...
    return createAsyncThunk<Returned, ThunkArg, AsyncThunkConfig>(
      ...
      async (arg, thunkAPI) => {
        try {
          const addressOrName =
            (await contractAddress(thunkAPI, arg)) ||
            (args as any)?.addressOrName;
          if (!addressOrName) {
          ...
          const network = networksConfig.find(
          ...
          const contractProvider = stateActions.provider(addressOrName, {
